        ...
        this.game.queryCircle(this.x, this.y, this.radius, (entity) => {
          if (entity.isStructure && entity.isConstructed) {
            if (entity.isCity) {
              ...
              if (entity.isMine) {
                totalResourceDemand[entity.resourceIndex] +=
                  entity.adjustedGenerationRate;
              }
            }
          }
        });
  ...
  render(ctx, dt, layerDepth) {
    ...
    if (layerDepth === this.renderLayers[0]) {
      ...
      if (settings.fancyPlanets) {
        ...
        let atmosphereGradient = ctx.createRadialGradient(
          0,
          0,
          0,
          0,
        ...
        atmosphereGradient.addColorStop(
          1 - stopSpacing * 2,
          utils.getAtmosphereColor(0.01)
        );
        atmosphereGradient.addColorStop(
          1 - stopSpacing,
          utils.getAtmosphereColor(0.07)
        );
      ...
      if (settings.planetProps) {
        ...
        for (let i = 0; i < propCount; i++) {
          // Get the prop ID
          let propId =
            planet.props[Math.floor((i / propCount) * planet.props.length)];
          ...
          let propSize =
            prop.sizes[
              Math.floor(utils.getRandomConst(i, 1) * prop.sizes.length)
            ];
          let propOffset =
            propSize / 2 - (propSize / 4) * utils.getRandomConst(i, 2);
          ...
          if (
            !this.game.pointVisible(this.x + propX, -this.y + propY, propSize)
          )
            continue;
      ...
      if (settings.planetProps) {
        ...
        for (let i = 0; i < cloudCount; i++) {
          // Position and size the tree
          let moveSpeed =
            ((utils.getRandomConst(i, 1) - 0.5) / planetCircumference) * 30;
          let moveOffset = moveSpeed * (Date.now() / 1000);
          let cloudAngle =
            ((utils.getRandomConst(i, 0) + i + moveOffset) % 1) * Math.PI * 2;
          let cloudIndex = (i % 3) + 1;
          let cloudOffset =
            this.atmosphereSize * (0.5 + utils.getRandomConst(i, 2) * 0.25);
          ...
          // Make sure the point is visible
          if (!this.game.pointVisible(this.x + cloudX, -this.y + cloudY, 64))
            continue;
          ...
          utils.drawImage(
            ctx,
            assets.propUrl(`cloud-${cloudIndex}`),
            0,
            0,
            cloudSize,
            cloudSize
        ...
        for (let i = 0; i < humanCount; i++) {
          ...
          let humanAngle =
            ((utils.getRandomConst(i, 2) + i + moveOffset) % 1) * Math.PI * 2;
          let bobbleMultiplier = isVehicle ? 2 : 10;
          let humanBobble = Math.sin(
            (Date.now() / 1000) * moveSpeed * bobbleMultiplier
          ); // Faster the move speed, faster they move up and down
          ...
          // Make sure the point is visible
          if (!this.game.pointVisible(this.x + humanX, -this.y + humanY, 64))
            continue;
