...
class Bullet extends Entity {
  ...
  get life() {
    return (
      (this.bulletPrefab.life || 0.8) *
      this.getPerkMultiplier(this.bulletPrefab.normalizedId + "Life")
    );
  }
  ...
  get explosionRadius() {
    return (
      (this.bulletPrefab.explosionRadius || 0) *
      this.getPerkMultiplier("explosionRadius")
    );
  }
  get explosionSpeed() {
  ...
  update(dt) {
    ...
    // Update delay
    this.bulletAccelerationTimer = Math.max(
      this.bulletAccelerationTimer - dt,
      0
    );
  ...
  onInit() {
    ...
    if (
      config.isServer &&
      this.shooterEntity.isTurret &&
      this.shooterEntity.structure.planetItem
    ) {
      ...
      this.game.spawnExplosion(
        ...
        (e) => {
          ...
          if (
            clientOwner &&
            this.shooterClient &&
            this.shooterClient.isFriendly(clientOwner.id)
          )
          ...
          if (
            ...
            return; // Don't damage structures from turrets
          return true;
        }
      );
    }
  ...
  render(ctx, dt) {
    ...
    if (this.blinkInterval !== -1) {
      ctx.globalAlpha =
        (this.renderLife / this.blinkInterval) % 1 > 0.5 ? 1 : 0.75;
    }
    ...
    if (
      ...
      // Draw trail
      let imageTrailLength =
        this.renderType === "trail" ? this.trailLength * 6 : 30;
