...
class Player extends Entity {
  ...
  get fireInterval() {
    ...
    return (
      weapon.fireInterval *
      this.getPerkMultiplier(weapon.normalizedId + "Interval")
    );
  }
  ...
  getMaxAmmo(weaponIndex) {
    ...
    return Math.floor(
      weapon.maxAmmo * this.getPerkMultiplier(weapon.normalizedId + "Ammo")
    );
  }
  ...
  constructor(game) {
    ...
    if (config.isClient) {
      ...
      this.thrustSprintSoundId = assets.playSound(
        "thrust-sprint",
        0,
        0,
        0,
  ...
  update(dt) {
    ...
    if (this.sprinting && this.moveSpeed > 0.1) {
      ...
      this.isLanded ||
      (Date.now() - this.lastSprintTime) / 1000 >= this.sprintRegenWait
    ) {
    ...
    if (
      config.isServer &&
      (this.isLanded ||
        (Date.now() - this.lastDamageTime) / 1000 >= this.healWaitTime)
    ) {
    ...
    let thrustMultiplier =
      this.moveSpeed *
      (isSprinting
        ? config.playerSprintingAcceleration *
          this.getPerkMultiplier("boostSpeed")
    ...
    if (config.isClient) {
      let lerpSpeed = 4;
      let maxVolume =
        this.game.playerId === this.id || this.game.spectateId === this.id
          ? 0.2
          : 0.4; // Make main player quieter
  ...
  onCollision(entity) {
    if (entity.isPlayer) {
      ...
      if (config.isServer) {
        ...
        if (entity.isBullet && this.isFriendlyClientId(entity.shooterClientId))
          shouldDamage = false;
  ...
  onKilled(killer) {
    ...
    if (this.clientHandle && killerLabel) {
      this.clientHandle.sendNotification(
        config.notificationTypes.DEATH,
        "notification-death",
        [killerLabel]
      );
    }
  ...
  collisionPerkDamage(target) {
    ...
    if (target.health != null) {
      ...
      let damageAmount =
        utils.dist(this.velX, this.velY, target.velX, target.velY) *
        collisionDamagePerVelocity;
      ...
      if (this.allianceId) {
        let allianceData =
          this.game.allianceList[
            this.game.allianceList.findIndex((a) => a[0] === this.allianceId)
          ];
  ...
  render(ctx, dt) {
    ...
    if (this.shieldTimer > 0) {
      ...
      if (this.shieldTimer < 2)
        ctx.globalAlpha = this.shieldTimer % 0.3 > 0.15 ? 1.0 : 0.5; // Blink if needed
    ...
    if (this.moveSpeed > 0.1) {
      ...
      let fireDistance =
        this.ship.fireDistance != null ? this.ship.fireDistance : 0.95;
      ctx.save();
      ctx.translate(0, this.radius * fireDistance);
      ctx.fillStyle = "#8C090A";
      this.drawFire(
        ctx,
        fireDepth * utils.lerp(0.7, 1.0, Math.random()),
        fireWidth
      );
      ctx.fillStyle = "#F4892E";
      this.drawFire(
        ctx,
        fireDepth * utils.lerp(0.4, 0.7, Math.random()),
        fireWidth
      );
      ctx.fillStyle = "white";
      this.drawFire(
        ctx,
        fireDepth * utils.lerp(0.2, 0.4, Math.random()),
        fireWidth
      );
      ctx.restore();
    }
    ...
    ctx.fillText(
      this.username,
      totalWidth - usernameWidth / 2 - totalWidth / 2,
      usernameY
    );
