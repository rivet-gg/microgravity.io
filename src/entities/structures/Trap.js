...
class Trap extends Structure {
    ...
    onCollision(entity) {
        // Trap the entity
        if (config.isServer && this.isConstructed && !this.trappedId && entity.isPlayer && !entity.isFriendlyClientId(this.clientOwnerId)) {
    ...
    render(ctx) {
        ...
        let isFriendly = this.game.currentPlayer && this.game.currentPlayer.isFriendlyClientId(this.clientOwnerId);
...
Trap.addSyncKeys(
    { key: "trappedId", type: "int", update: true }
);
