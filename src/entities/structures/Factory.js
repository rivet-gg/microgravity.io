...
class Factory extends WorkingStructure {
  ...
  onKilled(killer) {
    ...
    if (client) {
      ...
      for (let i = 0; i < this.depositedResources.length; i++) {
        client.giveResources(
          i,
          this.depositedResources[i] * config.mineAndFactoryTransferOnDeath
        );
      }

      // Give client weapons
      client.giveAmmo(
        this.structure.weaponIndex,
        this.count * config.mineAndFactoryTransferOnDeath
      );
    }
