...
class Structure extends Entity {
  ...
  update(dt) {
    ...
    if (config.isServer) {
      this.health = Math.min(
        this.health + this.healthRegenSpeed * dt,
        this.adjustedMaxHealth
      );
    }
    ...
    if (this.constructionTimer > 0) {
      ...
      // Call finish event
      if (this.constructionTimer <= 0 && config.isServer)
  ...
  onCollision(entity, dt) {
    if (entity.isPlayer || entity.isAsteroid) {
      ...
      let damageMultiplier =
        this.structure.planetItem && entity.explosionRadius === 0 ? 0.5 : 1.0;
  ...
  applyConstructionStyle(ctx) {
    if (!this.isConstructed) {
      let progress = utils.easingFunctions.easeOutQuad(
  ...
  postRender(ctx) {
    ...
    if (!this.isConstructed) {
      ...
      ctx.arc(
        ...
        radius,
        angleOffset,
        angleOffset + Math.PI * 2 * this.constructionProgress
      );
      ctx.fill();
    }
