...
class Wall extends Structure {
      ...
      this.game.queryMinMax(
        ...
        (entity) => {
          ...
          let sideSign = Math.sign(
            (x - this.x) * (entity.prevY - this.y) -
              (y - this.y) * (entity.prevX - this.x)
          );
          ...
          entity.setVelocity(
            Math.cos(moveAngle) * bounceVel,
            Math.sin(moveAngle) * bounceVel
          );
  ...
  reevaluateConnectedWalls() {
    ...
    this.game.queryCircle(
      ...
      (wall) => {
        ...
        if (
          !wall.clientOwner ||
          !this.clientOwner ||
          !wall.clientOwner.isFriendly(this.clientOwner.id)
        )
          return; // Make sure the owner is friendly
        connectableWalls.push([
          wall,
          utils.dist(this.x, this.y, wall.x, wall.y),
        ]);
      }
    );
    ...
    connectableWalls.length = Math.min(
      connectableWalls.length,
      config.connectedWallCount
    );
  ...
  render(ctx) {
    ...
    for (let [id, x, y] of this.connectedPositions) {
      ...
      let timeSinceBounce =
        (Date.now() - (this.lastBounceTimes[id] || 0)) / 1000;
      let bounceProgress =
        1 - Math.min(timeSinceBounce / this.bounceFadeTime, 1);
      bounceProgress = utils.easingFunctions.easeInQuint(bounceProgress);

      let lineStartRadius = this.radius - 2;
      ctx.save();
      ctx.strokeStyle = this.ownerAllianceId
        ? utils.getAllianceColor(this.ownerAllianceId)
        : "white";
