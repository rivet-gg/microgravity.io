...
class WorkingStructure extends Structure {
    ...
    set percentLaborFulfilled(v) { this._percentLaborFulfilled = v; }
    ...
    set adjustedGenerationRate(v) { this._adjustedGenerationRate = v; }
    ...
    canInteract(clientId) {
        ...
        if (this.allianceSharing) {
            if (config.isServer) {
                return this.clientOwner.alliance && this.clientOwner.alliance.clients.findIndex(m => m.id === clientId) !== -1;
            } else if (config.isClient) {
                return this.game.allianceData && this.game.allianceData[3].findIndex(m => m[0] === clientId) !== -1;
            }
        }
...
WorkingStructure.addSyncKeys(
    ...
    { key: "percentLaborFulfilled", type: "float", update: true, quantize: config.quantize01 },
    { key: "adjustedGenerationRate", type: "float", update: true, quantize: { min: 0, max: 1 << 8, bits: 8 } },
