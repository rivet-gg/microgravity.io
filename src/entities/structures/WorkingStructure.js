...
class WorkingStructure extends Structure {
  ...
  get percentLaborFulfilled() {
    ...
    if (this.hostPlanet.totalLaborDemand === 0 || this.laborDemand === 0) {
      ...
      let percentLaborDemand =
        this.laborDemand / this.hostPlanet.totalLaborDemand;
      let laborAllotted =
        percentLaborDemand * this.hostPlanet.totalLaborAvailable;
  ...
  get adjustedGenerationRate() {
    // Cached value
    if (this._adjustedGenerationRate != null)
      return this._adjustedGenerationRate;
  ...
  update(dt) {
    ...
    if (config.isServer && this.isConstructed) {
      ...
      if (generationSupply !== -1)
        generateCount = Math.min(generateCount, generationSupply); // Cap how much can be generated
    ...
    if (this.allianceSharing) {
      if (config.isServer) {
        return (
          ...
          this.clientOwner.alliance.clients.findIndex(
            (m) => m.id === clientId
          ) !== -1
        );
