...
class Turret extends Structure {
    ...
    render(ctx) {
        ...
        if (this.showBuildTips) {
            let color = "#D35E45";
            if (this.structure.planetItem) {
                utils.drawBuildTipCircle(ctx, color, 0, 0, this.viewDistance, this.id, -Math.PI / 2 - this.groundedAimRange, -Math.PI / 2 + this.groundedAimRange);
            } else {
...
Turret.addSyncKeys(
    { key: "aimDir", type: "float", update: true, sendMap: v => utils.modFix(v, Math.PI * 2) }
);
