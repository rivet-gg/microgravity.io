...
class Turret extends Structure {
  ...
  update(dt) {
    if (config.isServer && this.isConstructed) {
      ...
      if (
        (this.target && this.target.destroyed) ||
        this.game.updateIndex % 10 === 0
      ) {
        this.seekTarget();
      }
      ...
      if (this.target) {
        ...
        let bullet =
          bullets.bullets[
            bullets.bullets.findIndex((b) => b.id === this.weapon.bulletId)
          ];
  ...
  seekTarget() {
    ...
    this.game.queryCircle(this.x, this.y, this.viewDistance, (entity) => {
      ...
      if (entity.isPlayer && this.clientOwner.isFriendly(entity.clientId))
        return;
  ...
  canAimAt(angle) {
    ...
    if (this.structure.planetItem) {
      return (
        Math.abs(utils.dirDiff(angle + Math.PI / 2, this.rot)) <
        this.groundedAimRange
      );
    } else {
      return true;
    }
  ...
  render(ctx) {
    ...
    // Draw the base
    ctx.save();
    ctx.fillStyle = this.ownerAllianceId
    ...
    if (!this.structure.planetItem) {
      ...
      utils.drawImage(
        ctx,
        assets.iconUrl(this.weapon.icon),
        0,
        0,
        iconSize,
        iconSize
