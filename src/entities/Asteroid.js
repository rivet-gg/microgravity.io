        ...
        if (config.isServer) {
            ...
            if (killerClient) {
                ...
                killerClient.awardPoints(
                    this.isLarge ? config.pointRewardTypes.ASTEROID_LARGE : config.pointRewardTypes.ASTEROID_SMALL,
                ...
                let resourceCount = Math.floor(utils.lerp(config.asteroidMinResources, this.isLarge ? config.asteroidLargeMaxResources : config.asteroidSmallMaxResources, Math.random()));
...
Asteroid.addSyncKeys(
    ...
    { key: "rotVel", type: "float", update: false, quantize: { min: -config.maxAsteroidRotateSpeed, max: config.maxAsteroidRotateSpeed, bits: 7 } },
