...
module.exports = {
    ...
    entitiesCollide(entityA, entityB, useBodyRadius = true, padding = 0) {
        ...
        if (entityA.checkCircle && entityB.checkTrail) {
            return this.testCircleLine(entityA.x, entityA.y, radiusA + padding, entityB.x, entityB.y, entityB.prevX, entityB.prevY);
        }
        if (entityA.checkTrail && entityB.checkCircle) {
            return this.testCircleLine(entityB.x, entityB.y, radiusB + padding, entityA.x, entityA.y, entityA.prevX, entityA.prevY);
        }
    ...
    renderHealth(ctx, entity, maxHealth = 1.0, rotate = true) {
        ...
        ctx.fillRect(
            ...
            (width - padding) * adjustedHealth, (height - padding)
        );
    ...
    testCircleLine(circleX, circleY, circleRadius, lineX1, lineY1, lineX2, lineY2) {
        ...
        if (
            ...
            return true;
        } else if (this.dist(circleX, circleY, lineX1, lineY1) <= circleRadius || this.dist(circleX, circleY, lineX2, lineY2) <= circleRadius) {
    ...
    generateResourceHTML(index, value) {
        ...
        return `<div class="resourceIcon ${resource.icon}"></div><span>${typeof value === "number" ? Math.floor(value) : value}</span>`;
    },
     ...
     * = {x: 8, y: 8.5}
     */
    intercept(src, dst, v) {  // From: https://stackoverflow.com/a/3487761/2016800
        var tx = dst.x - src.x,
            ty = dst.y - src.y,
            tvx = dst.vx,
            tvy = dst.vy;
     ...
     */
    quad(a, b, c) {  // From: https://stackoverflow.com/a/3487761/2016800
        var sol = null;
        if (Math.abs(a) < 1e-6) {
            if (Math.abs(b) < 1e-6) {
    ...
    quantize(value, min, max, bits, ignoreWarning) {
        ...
        if (value < min || value > max) {
            if (!config.isProd) console.warn(`Attempting to quantize value ${value} outside of range [${min}, ${max}].`);
    ...
    numeralLookup: {M:1000,CM:900,D:500,CD:400,C:100,XC:90,L:50,XL:40,X:10,IX:9,V:5,IV:4,I:1},
    romanNumerals(num) {  // Adapted from https://stackoverflow.com/a/32851198
        let roman = "";
