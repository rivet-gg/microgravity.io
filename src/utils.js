...
module.exports = {
  ...
  entitiesCollide(entityA, entityB, useBodyRadius = true, padding = 0) {
    ...
    if (entityA.checkCircle && entityB.checkCircle) {
      return (
        this.dist(entityA.x, entityA.y, entityB.x, entityB.y) <=
        radiusA + radiusB + padding
      );
    }
  ...
  placeOnSurface(surface, entity) {
    ...
    entity.setVelocity(
      Math.cos(relativeAngle) * placeVelocity,
      Math.sin(relativeAngle) * placeVelocity
    );
  },
  ...
  generateUsername() {
    let prefix =
      config.playerNamePrefixes[
        Math.floor(Math.random() * config.playerNamePrefixes.length)
      ];
  ...
  renderHealth(ctx, entity, maxHealth = 1.0, rotate = true) {
    ...
    let adjustedHealth = this.easingFunctions.easeInQuad(
  ...
  constructElements(...elementArray) {
    for (let i = 0; i < elementArray.length; i++) {
      ...
      if (!(element instanceof Node))
        elementArray[i] = document.createTextNode(element); // Create node out of element
    }
  ...
  grayscaleColor(grayscale, alpha = 1.0) {
    grayscale *= 255;
    return `rgba(${Math.floor(grayscale)}, ${Math.floor(
      grayscale
    )}, ${Math.floor(grayscale)}, ${alpha})`;
  },
  ...
  getAtmosphereColor(l = 1.0) {
    return `rgb(${Math.floor(64 * l)}, ${Math.floor(162 * l)}, ${Math.floor(
      236 * l
    )})`;
  },
  ...
  assert(condition, message = null) {
    if (!condition)
      throw new Error(
        message ? `Failed assertion: ${message}` : "Failed assertion."
      );
  },
  ...
  drawImage(ctx, path, x, y, width, height, tint = null) {
    ...
    ctx.drawImage(
      assets.getImage(path, width, height, tint),
      x - width / 2,
      y - height / 2
    );
  },
  ...
  quantize(value, min, max, bits, ignoreWarning) {
    ...
    if (
      !ignoreWarning &&
      !config.isProd &&
      this.suggestedQuantizationLevels.indexOf(bits) === -1
    ) {
    ...
    if (value < min || value > max) {
      if (!config.isProd)
        console.warn(
          `Attempting to quantize value ${value} outside of range [${min}, ${max}].`
        );
  ...
  rankForScore(score) {
    return (
      Math.pow(score / config.rankTarget, 1 / config.rankExponential) *
      config.rankScale
    );
  },
  ...
  scoreForRank(rank) {
    return (
      config.rankTarget *
      Math.pow(rank / config.rankScale, config.rankExponential)
    );
  },
