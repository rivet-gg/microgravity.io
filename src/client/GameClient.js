...
class GameClient extends Game {
  ...
  get buildTipSlot() {
    return this.vue.hoveringStructure
      ? structures.structureForId(this.vue.hoveringStructure).slot
      : null;
  }

  get averageFPS() {
    return (
      this.framesPerSecondHistory.reduce((n, v) => n + v) /
      this.framesPerSecondHistory.length
    );
  }
  ...
  get isFuckNathan() {
    return (
      this.normalizedUsername === "#fucknathan" ||
      this.normalizedUsername === "#fuckingnathan"
    );
  }
  ...
  initUI() {
    ...
    let languageOptions = navigator.languages
      ? navigator.languages
      : [navigator.language]; // Fallback to default language
    ...
    this.vue = new Vue({
      ...
      data: {
        ...
        // Ships
        selectedShip:
          localStorage.getItem("selectedShip") || config.ships[0].id,
        selectedFill:
          localStorage.getItem("selectedFill") || config.shipFills[0].value,
      ...
      methods: {
        ...
        purchaseStructure(structureId) {
          game.sendPurchaseStructure(
            structures.structureIndexForId(structureId)
          );
        },
        ...
        isShipLocked(ship) {
          return (
            ship.reward != null && this.rewards.indexOf(ship.reward) === -1
          );
        },
        selectShip(ship) {
      ...
      computed: {
        ...
        buildableStructures() {
          return this.structureList.filter(
            (s) =>
              (this.isLanded && s.planetItem) ||
              (!this.onPlanet && !s.planetItem)
          );
        },
        ...
        // Upgrades
        upgradeOptionsData() {
          return this.upgradeOptions != null
            ? upgrades.upgrades[this.upgradeOptions]
            : null;
        },
        hoveringPerkData() {
          return this.hoveringPerk != null
            ? upgrades.perks[this.hoveringPerk]
            : null;
        },
      },
    ...
    if (config.isProd)
      window.onbeforeunload = () =>
        "Are you sure you want to leave? You will lose all progress.";
    ...
    document
      .getElementById("createAllianceButton")
    ...
    document
      .getElementById("leaveAllianceButton")
      .addEventListener("click", () => {
    ...
    // Buttons
    document
      .getElementById("showChatButton")
      .addEventListener("click", () => this.toggleChatBox(true));
    document
      .getElementById("showHelpButton")
      .addEventListener("click", () => this.toggleHelp(true));
    document
      .getElementById("dismissHelp")
    ...
    document.addEventListener(
      ...
      true
    ); // Enable `:active` in CSS (https://css-tricks.com/snippets/css/remove-gray-highlight-when-tapping-links-in-mobile-safari/)
    ...
    this.canvas.addEventListener(
      "touchstart",
      (e) => this.canvasTouchStart(e),
      false
    );
    this.canvas.addEventListener(
      "touchmove",
      (e) => this.canvasTouchMove(e),
      false
    );
    this.canvas.addEventListener(
      "touchend",
      (e) => this.canvasTouchEnd(e),
      false
    );
    this.canvas.addEventListener(
      "touchcancel",
  ...
  initInput() {
    ...
    document.addEventListener("mousewheel", (e) =>
      this.switchWeaponBy(Math.sign(e.wheelDelta))
    );
    InputHandler.subscribe(
      ["q"],
      () => !this.inputFocused && this.switchWeaponBy(-1)
    );
    InputHandler.subscribe(
      ["e"],
      () => !this.inputFocused && this.switchWeaponBy(1)
  ...
  resize() {
    ...
    this.pixelDensity = settings.nativeResolution
  ...
  switchWeaponBy(dir) {
    ...
    dir = Math.sign(dir);
    let currentIndex = this.ownedWeapons.findIndex(
      (w) => w[0] === player.weaponIndex
    );
    let selectedIndex = utils.clamp(
      currentIndex + dir,
      0,
      this.ownedWeapons.length - 1
    );
  ...
  update(dt) {
    ...
    if (this.currentPlayer) {
      this.queryCircle(
        ...
        (entity) => {
          if (
            entity.isBullet &&
            !this.currentPlayer.isFriendlyClientId(entity.shooterClientId)
          ) {
    ...
    this.vue.onPlanet =
      this.currentPlayer && this.currentPlayer.overlappingPlanet != null;
    ...
    // Update Vue selected
    this.vue.selectedWeaponIndex = this.currentPlayer
      ? this.currentPlayer.weaponIndex
      : -1;
    this.vue.ownedWeaponIndex = this.currentPlayer
      ? this.ownedWeapons.findIndex(
          (w) => w[0] === this.currentPlayer.weaponIndex
        )
      : -1;
  }
  ...
  render() {
    ...
    if (!!spectatingPlayer) {
      if (this.currentPlayer) {
        ...
        this.viewportX = utils.lerp(
          this.viewportX,
          spectatingPlayer.x,
          screenLerpSpeed
        );
        this.viewportY = utils.lerp(
          this.viewportY,
          -spectatingPlayer.y,
          screenLerpSpeed
        ); // Flip Y coords
    ...
    assets.soundVol(
      this.ambientSoundId,
      (1 - this.drumsProgress) * this.maxAmbientVolume
    );
  ...
  renderWorld(ctx, dt) {
    ...
    let shakeMagnitude = this.explosionManager.getScreenShake(
      this.viewportX,
      this.viewportY
    );
    ...
    let shakenViewportX =
      this.viewportX + (Math.random() - 0.5) * shakeMagnitude;
    let shakenViewportY =
      this.viewportY + (Math.random() - 0.5) * shakeMagnitude;
    ...
    for (let entityId in this.chatMessages) {
      ...
      let [text, clientId, username, msgX, msgY, sendTime] =
        this.chatMessages[entityId];
      ...
      if (entityVisible) {
        ...
        this.allianceData &&
        this.allianceData[3].findIndex((c) => c[0] === clientId) !== -1
      ) {
        // Render from afar if in alliance
        let messageAngle = Math.atan2(
          msgY - this.viewportY,
          msgX - this.viewportX
        );
        let messageDistance =
          (config.viewportHeight * this.viewDistanceMultiplier) / 2 - 200;
  ...
  renderUI(ctx, dt) {
    ...
    if (this.currentPlayer) {
      ...
      vignetteDistance = utils.lerp(
        vignetteDistance,
        config.lowHealthVignetteDistance,
        1 - health
      );
    ...
    vignetteGradient.addColorStop(
      1.0 - vignetteDistance,
      `rgba(${vignetteColor.join(",")})`
    );
    ...
    if (this.lastNotificationDate) {
      ...
      let outroProgress = utils.clamp01(
        (notificationLife - timeSince) / fadeSpeed
      );
  ...
  renderMinimap(ctx) {
    ...
    if (this.currentPlayer) {
      ...
      ctx.save();
      ctx.fillStyle = this.allianceData
        ? utils.getAllianceColor(this.allianceData[0])
        : "white";
      ctx.translate(x, y);
      ctx.rotate(this.currentPlayer.rot);
      ctx.beginPath();
  ...
  renderTextButton(
    ...
    let textWidth =
      width === -1 ? ctx.measureText(text).width + paddingX * 2 : width;
    let textHeight =
      height === -1
        ? parseFloat(ctx.font.split("px")[0]) + paddingY * 2
        : height;
  ...
  onLeaderboard(data) {
    ...
    let itemCount =
      ...
      (this.playerRank >= this.leaderboardData.length ? 1 : 0);
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      let allianceData =
        allianceId != null
          ? this.allianceList.filter((a) => a[0] === allianceId)[0]
          : null;
      ...
      leaderboardItems.push({
        ...
        allianceColor: allianceData
          ? utils.getAllianceColor(allianceData[0])
          : null,
  ...
  onMinimap(minimap) {
    ...
    for (let value of minimap) {
      value[2] = utils.unquantize(
        value[2],
        -mapRadius,
        mapRadius,
        config.minimapPositionBits
      ); // X pos
      value[3] = utils.unquantize(
        value[3],
        -mapRadius,
        mapRadius,
        config.minimapPositionBits
      ); // Y pos
      value[4] = utils.unquantize(
        value[4],
        0,
  ...
  onWeapons(data) {
    ...
    for (let i = 0; i < this.ownedWeapons.length; i++) {
      ...
      // Update item
      let maxAmmo = this.currentPlayer
        ? this.currentPlayer.getMaxAmmo(weaponIndex)
        : weapon.maxAmmo;
  ...
  onNotification(data) {
    ...
    for (let i = 0; i < translationData.length; i++) {
      if (translationData[i].startsWith("structure:")) {
        translationData[i] = structures.structureForId(
          translationData[i].slice(10)
        ).name;
      }
    }
    ...
    this.lastNotificationText = utils.translate(
      translationKey,
      ...translationData
    );
  }
  ...
  updateGameState() {
    ...
    // Show/hide menu
    let showingLoading =
      (!this.socketOpen || !this.initiated) && !this.socketClosed;
    document.getElementById("loadingScreen").style.display = showingLoading
      ? "flex"
      : "none";
    let showingDisconnect = this.socketClosed;
    document.getElementById("disconnectScreen").style.display =
      showingDisconnect ? "flex" : "none";
  ...
  toggleAlliances(show) {
    ...
    if (show) {
      ...
      while (allianceHolder.firstChild)
        allianceHolder.removeChild(allianceHolder.firstChild);
      ...
      document.getElementById("createAllianceHolder").style.display = this
        .allianceData
        ? "none"
        : "block";
      document.getElementById("leaveAllianceButton").style.display = this
        .allianceData
        ? "block"
        : "none";
    }
  ...
  updateControlPanel() {
    ...
    if (planet) {
      ...
      for (let i = 0; i < planet.resources.length; i++) {
        let generationRate =
          planet.resourceGenerationRate[i] - planet.totalResourceDemand[i];
        resourceValues.push(
          `<span class="${
            planet.resources[i] < 10 ? "warning" : ""
          }">${Math.floor(planet.resources[i])}/${
            planet.resourceMax[i]
          }</span> (<span class="${
    ...
    if (structure) {
      ...
      // Set name
      document.getElementById("structurePanelTitle").innerText =
      ...
      if (structure.isWorkingStructure) {
        ...
        addDetails(
          ...
          `${utils.translate("count")}: ${Math.floor(
            structure.count
          )}/${Math.floor(structure.maxCount)}`
        );
      ...
      if (structure.canInteract(this.clientId)) {
        if (structure.isMine) {
          ...
          for (let i = 0; i < structure.depositedResources.length; i++) {
            ...
            resourceStats.push(
              ...
              }">${Math.floor(deposited)}</span> (${bulletResource.toFixed(
                1
              )}/unit)`
            );
          }
          ...
          if (produceCount > 0) {
            ...
            addDetails(
              "factoryProductCount",
              utils.translate("out-of-resources")
            );
          }
          ...
          addAction(
            "factoryCollectCount",
            `${utils.translate("collect-label")} ${collectCount} ${
          ...
          addAction(
            "factoryCollectAll",
            `${utils.translate("collect-label")} ${utils.translate(
              "all-label"
            )} ${structure.weapon.name}`,
            () => {
              this.sendStructureAction(structures.actions.COLLECT, allCount);
        ...
        if (structure.isOwner(this.clientId)) {
          ...
          if (!this.demolishConfirm) {
            ...
            addAction(
              "demolish",
              `${utils.translate("confirmation-question")}`,
              () => {
  ...
  renderServerList(options) {
    ...
    while (serverSelector.firstChild)
      serverSelector.removeChild(serverSelector.firstChild);
