...
assets.calculateSoundVolume = function (baseVolume, x, y) {
  if (x != null && y != null) {
    ...
    let distance = Math.sqrt(
      Math.pow(this.soundListenerY - y, 2) +
        Math.pow(this.soundListenerX - x, 2)
    );

    // Calculate the volume based on distance
    let distanceProgress =
      (distance - this.soundRefDistance) /
      (this.soundMaxDistance - this.soundRefDistance); // Volume based on distance
...
if (config.isClient) {
  // Play ambient noise
  assets.ambientSoundId = assets.playSound(
    "ambient",
    this.maxAmbientVolume,
    null,
    null,
    true
  );
  assets.drumSoundId = assets.playSound(
    "drums",
    this.maxDrumsVolume,
    null,
    null,
