...
assets.sound = config.isClient && new Howl({
...
assets.playSound = function(id, volume = 1.0, x = null, y = null, loop = false) {
    try {
        ...
        let skipSound = assets.positionalAudio ? false : (!loop && this.calculateSoundVolume(volume, x, y) < 0.01);
        if (skipSound) return;
        ...
        if (assets.positionalAudio && x != null && y != null) this.sound.pannerAttr(assets.pannerAttr, soundId);
...
assets.calculateSoundVolume = function(baseVolume, x, y) {
    if (x != null && y != null) {
        ...
        let distanceProgress = (distance - this.soundRefDistance) / (this.soundMaxDistance - this.soundRefDistance);  // Volume based on distance
...
assets.soundRate = function(rate, id) {
    try { this.sound.rate(rate, id); } catch(e) { console.error("HowlerJS Error:", e); }
};

assets.soundStop = function(id) {
    try { this.sound.stop(id); } catch(e) { console.error("HowlerJS Error:", e); }
};
