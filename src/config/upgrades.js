...
upgrades.upgrades = [
    [ { structureId: "launch-pad" }, { structureId: "shotgun-factory" }, { structureId: "super-minigun-factory" } ],
    [ { moveSpeed: 1.15, label: "upgrade-move-speed" }, { boostSpeed: 1.2, label: "upgrade-boost-speed" }, { boostAmount: 1.4, label: "upgrade-boost-fuel" } ],  // Testing Tier
    [ { bombInterval: 0.65, bombAmmo: 1.5, label: "upgrade-bomb" }, { machineGunInterval: 0.90, minigunInterval: 0.90, minigunAmmo: 2, superMinigunAmmo: 4, label: "upgrade-minigun" }, { structureId: "ground-sniper-turret" } ],  // Tier 1  // { buildTime: 0.1 }
    [ { collisionDamage: 1 / 1400, label: "upgrade-collision-damage" }, { viewDistance: 1.05, label: "upgrade-view-distance" }, { structureId: "ground-missile-turret" }  ],  // Tier 2  // { cityPopulation: 1.5 }
    ...
    [ { missileInterval: 0.75, missileAmmo: 1.5, label: "upgrade-missile" }, { machineGunDamage: 1.08, minigunDamage: 1.1, label: "upgrade-machine-gun" }, { structureId: "ied" } ],  // Tier 3  // { cost: 0.9 }
    [ { structureId: "anti-follow-factory" }, { structureId: "minigun-turret" }, { structureId: "ground-minigun-turret" } ],
    [ { structureId: "super-minigun-turret"}, { explosionRadius: 1.5, label: "upgrade-explosion-radius" }, { machineGunLife: 1.05, minigunLife: 1.05, shotgunLife: 1.3, label: "upgrade-weapon-range" } ],  // Tier 4  // { turretBulletLife: 1.5 }
    [ { structureId: "heal-factory" }, { structureId: "teleport-factory" }, { structureId: "shield-factory" } ],
    [ { structureId: "boost-pad" }, { structureId: "trap" } ],
    [ { structureId: "nuke-factory" }, { shotgunAmmo: 3, minigunAmmo: 3, antiFollowAmmo: 3, sniperAmmo: 3, label: "upgrade-tripple-ammo" } ],  // Tier 5  // { /* Max turret max count */ }
];
