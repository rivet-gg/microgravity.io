...
class ClientHandle {
  ...
  update(dt) {
    ...
    if (
      this.pingStart != null &&
      Date.now() - this.pingStart > config.pingTimeout
    ) {
      this.pingStart = null;
    ...
    if (this.player && !this.player.destroyed) {
      ...
      if (this.kickTimer > config.kickTimeout) {
        if (!config.isProd)
          console.log(`Kicking click id ${this.id} for inactivity.`);
        this.ws.close();
        return;
      }
    }
    ...
    if (this.rankFloor > this.notifiedRank) {
      ...
      this.sendNotification(
        config.notificationTypes.RANK_UP,
  ...
  sendUpdate() {
      ...
      (entity) => {
        ...
        if (visibleEntityIndex !== -1) {
          ...
          if (entity.sendUpdates) {
            updatedEntities[entity.id] = this.game.serializeEntity(
              entity.id,
              false
            );
          }
          ...
          // Add appeared entity
          appearedEntities[entity.id] = this.game.serializeEntity(
            entity.id,
            true
          );
        }
    ...
    for (let entityId of oldVisibleEntities) {
      ...
      if (entity && !entity.destroyed) {
        ...
        if (!destroyedEntity)
          console.warn(`Failed to find destroyed entity for ID ${entityId}.`);
        destroyedEntities.push([
          entityId,
          destroyedEntity && destroyedEntity.destroyedData,
        ]);
      }
    }
  ...
  sendLeaderboard(leaderboard) {
    ...
    // Send to client
    this.send(config.serverMessages.LEADERBOARD, [
  ...
  sendTextPopup(text, x, y, icon = null, life = null) {
    this.send(config.serverMessages.TEXT_POPUP, [
      text,
      x,
      y,
      icon,
      utils.clamp(life, 0.5, 15),
    ]);
  }
  ...
  sendNotification(type, translationKey, translationData) {
    this.send(config.serverMessages.NOTIFICATION, [
      type,
      translationKey,
      translationData,
    ]);
  ...
  sendResources() {
    this.send(config.serverMessages.RESOURCES, [
      this.score,
      this.resources,
  ...
  onJoin(data) {
    ...
    if (!foundSpawnPosition)
      [this.player.x, this.player.y] = this.game.chooseSpawnPoint(
        this.player.radius,
        300
      );
  ...
  onInput(data) {
    let [moveDir, moveSpeed, relativeMoveDir, targetRot, firing, sprinting] =
      data;
    ...
    if (text.startsWith("/")) {
      ...
      switch (command) {
        ...
        case "t":
          ...
          this.sendSelfMessage(
            `Teleported to ${xPos.toFixed(1)} ${yPos.toFixed(1)}.`
          );
        ...
        case "kick":
          ...
          let client =
            this.game.clients[
              this.game.clients.findIndex((c) => c.id === clientId)
            ];
      ...
      case structures.actions.COLLECT:
        ...
        if (structure.isMine) {
          ...
          this.giveResources(
            ...
            count,
            structure.x -
              Math.cos(structure.rot + Math.PI / 2) * structure.radius * 0.5,
            structure.y +
              Math.sin(structure.rot + Math.PI / 2) * structure.radius * 0.5
          );
          this.sendResources();
          ...
          count = Math.min(
            count,
            structure.weapon.maxAmmo - this.getAmmoCount(weaponIndex)
          );
  ...
  beforeFire() {
    ...
    if (ammo === 0) {
      ...
      let ownedWeaponIndex = this.ownedWeapons.findIndex(
        (w) => w[0] === this.player.weaponIndex
      );
  ...
  killedPlayer(victim) {
    ...
    if (victim.clientHandle) {
      ...
      for (let i = 0; i < this.resources.length; i++) {
        let resources =
          victim.clientHandle.resources[i] * config.playerDeathScoreLoss;
    ...
    this.awardPoints(
      config.pointRewardTypes.PLAYER_KILL,
      points,
      victim.x,
      victim.y
    );
    ...
    // Send notification
    this.sendNotification(
      config.notificationTypes.PLAYER_KILL,
      "notification-player-kill",
  ...
  awardPoints(type, count, x = null, y = null, appendText = "") {
    ...
    // Send to client
    if (x != null && y != null)
      this.sendPointsPopup(`+${Math.floor(count)}${appendText}`, x, y);
    this.sendResources();
  ...
  attemptPurchase(resources) {
    if (this.canAfford(resources)) {
      ...
      this.sendNotification(
        config.notificationTypes.CANT_AFFORD,
        "notification-cant-afford"
      );
      return false;
    }
  }
  ...
  isFriendly(clientId) {
    ...
    return (
      this.alliance &&
      this.alliance.clients.findIndex((c) => c.id === clientId) !== -1
    );
  }
