...
class GameServer extends Game {
  ...
  generatePlanets() {
    ...
    centerPlanet.resourceMax = centerPlanet.resourceMax.map(
      () => config.centerPlanetResourceMap
    );
    centerPlanet.resourceGenerationRate =
      centerPlanet.resourceGenerationRate.map(
    ...
    for (let i = 0; i < planetCount; i++) {
      ...
      planet.radius = utils.lerp(
        minPlanetRadius,
        maxPlanetRadius,
        radiusProgress
      );
      planet.atmosphereSize = Math.max(
        config.minPlanetAtmosphereSize,
        planet.radius * 0.2
      );

      // Choose max resources; this will choose max values that will all add up to `maxResourceMax`
      let totalResourceMax = utils.lerp(
        minTotalResourceMax,
        maxTotalResourceMax,
        1 - distanceProgress
      );
      planet.resourceMax = planet.resourceMax.map(() => Math.random());
      let resourceMaxMultiplier =
        (1 / planet.resourceMax.reduce((t, n) => t + n)) * totalResourceMax;
      planet.resourceMax = planet.resourceMax.map((v) =>
        Math.floor(v * resourceMaxMultiplier)
      );

      // Choose generation rate; same as for max values
      let totalGenRate = utils.lerp(
        minTotalGenRate,
        maxTotalGenRate,
        1 - distanceProgress
      );
      ...
      let resourceGenRateMultiplier =
        (1 / planet.resourceGenerationRate.reduce((t, n) => t + n)) *
        totalGenRate;
      planet.resourceGenerationRate = planet.resourceGenerationRate.map(
      ...
      while (true) {
        ...
        this.queryBound(planet.bound, (entity) => {
          // Flag as collides
          doesCollide =
            doesCollide || utils.entitiesCollide(entity, planet, false);
        });
